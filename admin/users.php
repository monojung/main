...
try {
    ...
    // Departments table might not exist - create default departments
    $departments = [
        ['id' => 1, 'name' => 'แผนกบริหาร'],
        ['id' => 2, 'name' => 'แผนกการพยาบาล'],
        ['id' => 3, 'name' => 'แผนกเวชกรรม'],
        ['id' => 4, 'name' => 'แผนกเทคนิคการแพทย์'],
        ['id' => 5, 'name' => 'แผนกเภสัชกรรม']
    ];
}
...
if (empty($action)) {
    ...
    // Get records with department name if possible
    try {
        $query = "
        ...
        // If departments table doesn't exist, get users without department info
        $query = "
            SELECT u.*, '' as department_name 
            FROM users u 
            WHERE $where_clause
            ORDER BY u.created_at DESC 
            LIMIT $per_page OFFSET $offset
                ...
                <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-sm text-gray-600">
                            แสดง <?php echo number_format(count($users_list ?? [])); ?> จาก <?php echo number_format($total_records ?? 0); ?> รายการ
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    <?php echo isset($user['last_login']) && $user['last_login'] ? safeFormatThaiDateTime($user['last_login']) : 'ยังไม่เคยเข้าสู่ระบบ'; ?>
                                </td>
                                <td class="px-6 py-4">
            ...
            <div class="glass-card rounded-2xl p-8 fade-in">
                <form method="POST" class="space-y-6">
                    ...
                    <div class="bg-blue-50 p-6 rounded-xl">
                        <h4 class="text-lg font-semibold text-gray-800 mb-4">👤 ข้อมูลส่วนตัว</h4>
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (firstName && lastName && username && email) {
                        if (!firstName.value.trim() || !lastName.value.trim() || !username.value.trim() || !email.value.trim()) {
                            e.preventDefault();
